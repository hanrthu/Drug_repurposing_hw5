...
class HNM(nn.Module):
    ...
    virus_human,virus_human_norm,protein_drug,protein_drug_norm,drug_protein_mask):
        W_hd = human_drug_norm.clone()
        W_dd = drug_drug_norm
        W_dv = drug_protein_norm.clone() * drug_protein_mask
        W_vv = virus_virus_norm
        W_hh = human_human_norm

        for i in range(self.steps):
            print("Iteration:",i)
            W_hd = self.alpha * torch.matmul(torch.matmul(torch.matmul(torch.matmul(W_hd,W_dd),W_dv),W_vv),W_dv.T) + (1-self.alpha) * human_drug_norm
            W_dv = self.alpha * torch.matmul(torch.matmul(torch.matmul(torch.matmul(W_hd.T,W_hh),W_hd),W_dd),W_dv) + (1-self.alpha) * drug_protein_norm * drug_protein_mask
            # print(W_dv)
            # print(drug_protein_norm)
        return W_dv
